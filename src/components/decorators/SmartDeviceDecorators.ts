...
import { Decorator, IModelConnection, Marker, ScreenViewport, DecorateContext } from "@itwin/core-frontend";
...
export class SmartDeviceDecorator implements Decorator {
   ...
   private async getSmartDeviceData() {
      const query = `SELECT SmartDeviceId, 
      SmartDeviceType,
      ECInstanceId,
      Origin
   ...
   private async addMarkers() {
      ...
      values.forEach(value => {
         const SmartDeviceMaker = new SmartDeviceMarker({ x: value.origin.x, y: value.origin.y, z: value.origin.z },
             { x: 40, y: 40 }, 
             value.smartDeviceId, 
         ...
         this._markerSet.push(SmartDeviceMaker);
      })
   }
